...
## Prerequisites — refuse without these (P1)
Refuse-without-required-inputs: if a hard input below is missing → emit `[doc-audit-refused] reason:<X>` + name what is missing → halt. Do not audit partially.
- [ ] Target file path provided + exists on disk → missing: `[doc-audit-refused] reason:no-target` · ask for the file to audit
- [ ] Target is not a stub (≥20 lines of real content) → stub: `[doc-audit-refused] reason:stub` · ask to complete it first
