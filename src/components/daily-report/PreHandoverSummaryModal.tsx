...
export const PreHandoverSummaryModal: React.FC = () => {
  const {
    showPhSummaryModal,
  ...
  if (!showPhSummaryModal || !selectedPhCatInfo) return null;
  ...
  const internalCount = labor
    .filter(l => l.membership === 'Internal' && (l.shifts?.normal || l.shifts?.otMorning || l.shifts?.otNoon || l.shifts?.otEvening))
    .reduce((acc, l) => acc + (Number(l.amount) || 1), 0);
  ...
  const originalReport = isPhEditingExisting
    ? phDailyHistory.find((h: any) => h.id === reportDate || h.date?.split('T')[0] === reportDate)
    : null;
  ...
  const removedWorkers: any[] = [];
  if (originalReport) {
    for (const [wId, orig] of originalLaborMap.entries()) {
      if (!labor.some(l => (l.staffId || l.id) === wId)) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width: '580px', maxWidth: '100%', maxHeight: '90vh',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', borderBottom: '1px solid #f1f5f9', paddingBottom: '1rem' }}>
          ...
          <div style={{ flex: 1 }}>
            <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 900, color: '#0f172a' }}>
              {isPhEditingExisting ? 'ตรวจสอบการแก้ไขรายงานประจำวัน' : 'ตรวจสอบรายงานประจำวัน'}
            </h3>
          ...
          <button
            onClick={() => setShowPhSummaryModal(false)}
            style={{ border: 'none', background: 'none', color: '#94a3b8', cursor: 'pointer', padding: '4px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        ...
        {/* Scrollable body */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', overflowY: 'auto', paddingRight: '4px', maxHeight: 'calc(90vh - 200px)' }}>
          ...
          <div style={{
            ...
            padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '10px',
          }}>
            <h4 style={{ margin: 0, fontSize: '0.85rem', fontWeight: 800, color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              ข้อมูลการดำเนินงาน
              {isProgressChanged && (
                <span style={{ fontSize: '0.65rem', fontWeight: 800, padding: '2px 6px', borderRadius: '6px', background: '#ea580c', color: '#ffffff', marginLeft: '6px' }}>
                  แก้ไขความคืบหน้า
                </span>
              )}
            </h4>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.9rem', fontWeight: 800, color: '#2563eb' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b' }}>
                    ({progress === 100 ? 'ปิดงาน' : 'อัปเดตความคืบหน้า'})
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* Labor card */}
          <div style={{ background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <div style={{ maxHeight: '260px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px', paddingRight: '2px', marginTop: '4px' }}>
              {labor.map(l => {
                const activeShifts: { name: string; key: string }[] = [];
                ...
                const isNewWorker = isPhEditingExisting && originalReport && !orig;
                const isShiftChanged = isPhEditingExisting && originalReport && orig && (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                        <div style={{ width: 24, height: 24, borderRadius: 6, background: l.membership === 'Internal' ? '#eff6ff' : '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        ...
                        <div style={{ fontSize: '0.8rem', fontWeight: 800, color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'flex', alignItems: 'center' }}>
                          ...
                          {isNewWorker && <span style={{ fontSize: '0.6rem', fontWeight: 800, padding: '1px 5px', borderRadius: '4px', background: '#10b981', color: '#ffffff', marginLeft: '6px' }}>เพิ่มใหม่</span>}
                          {isShiftChanged && <span style={{ fontSize: '0.6rem', fontWeight: 800, padding: '1px 5px', borderRadius: '4px', background: '#ea580c', color: '#ffffff', marginLeft: '6px' }}>แก้ไขเวลา</span>}
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                        {activeShifts.map((sh, sIdx) => {
                          let bg = '#dbeafe'; let text = '#1e40af';
                          if (sh.name.startsWith('OT')) { bg = '#fef3c7'; text = '#92400e'; }
                          if (sh.name === 'ลางาน') { bg = '#fee2e2'; text = '#991b1b'; }
                          const isShiftAdded = isPhEditingExisting && originalReport && orig && (
                          ...
                          return (
                            <span key={sIdx} style={{ fontSize: '0.65rem', fontWeight: 800, padding: '2px 6px', borderRadius: '6px', background: bg, color: text, border: isShiftAdded ? '1.5px dashed #ea580c' : '1px solid transparent', display: 'inline-flex', alignItems: 'center', gap: '2px' }}>
                    ...
                    {removedShifts.length > 0 && (
                      <div style={{ display: 'flex', gap: '4px', alignItems: 'center', fontSize: '0.65rem', color: '#ef4444', fontWeight: 700, marginTop: '4px', paddingTop: '4px', borderTop: '1px dotted #fecaca' }}>
                        <span style={{ color: '#94a3b8' }}>นำออก:</span>
                        {removedShifts.map((sh, idx) => (
                          <span key={idx} style={{ background: '#fee2e2', color: '#b91c1c', padding: '1px 5px', borderRadius: '4px', textDecoration: 'line-through' }}>{sh}</span>
                        ))}
                      </div>
                    )}
                  </div>
                ...
                <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px dashed #fca5a5' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {removedWorkers.map(rw => {
                      ...
                      return (
                        <div key={rw.staffId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#fef2f2', borderRadius: '10px', border: '1px solid #fca5a5', opacity: 0.8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                            <div style={{ width: 24, height: 24, borderRadius: 6, background: '#fee2e2', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              <User size={12} color="#ef4444" />
                            </div>
                            <div style={{ fontSize: '0.8rem', fontWeight: 800, color: '#991b1b', textDecoration: 'line-through', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              {rw.employeeId ? `${rw.employeeId} : ` : ''}{rw.staffName}
                            </div>
                          </div>
                          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                            {origShifts.map((sh, idx) => (
                              <span key={idx} style={{ fontSize: '0.65rem', fontWeight: 800, padding: '2px 6px', borderRadius: '6px', background: '#fee2e2', color: '#991b1b', textDecoration: 'line-through' }}>{sh}</span>
                            ))}
                            {Number(rw.amount) > 1 && (
          ...
          <div style={{
            ...
            padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '8px',
          }}>
            <h4 style={{ margin: 0, fontSize: '0.85rem', fontWeight: 800, color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Info size={14} color={isNoteChanged ? '#ea580c' : '#64748b'} />
              หมายเหตุ (Site Notes)
              {isNoteChanged && (
                <span style={{ fontSize: '0.65rem', fontWeight: 800, padding: '2px 6px', borderRadius: '6px', background: '#ea580c', color: '#ffffff', marginLeft: '6px' }}>แก้ไขแล้ว</span>
              )}
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {isNoteChanged && originalReport.note && (
                <div style={{ fontSize: '0.75rem', color: '#94a3b8', textDecoration: 'line-through', background: '#fee2e2', padding: '6px 8px', borderRadius: '8px', border: '1px solid #fecaca' }}>
                  เดิม: {originalReport.note}
                </div>
              )}
              <p style={{ margin: '4px 0 0 0', fontSize: '0.8rem', fontWeight: note ? 700 : 500, color: note ? '#334155' : '#94a3b8', background: '#ffffff', padding: '10px 12px', borderRadius: '10px', border: '1px solid #f1f5f9', whiteSpace: 'pre-wrap', lineHeight: 1.4 }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '8px', borderTop: '1px solid #f1f5f9', paddingTop: '1.25rem' }}>
          <button
            onClick={() => setShowPhSummaryModal(false)}
            style={{ flex: 1, padding: '12px', borderRadius: '14px', border: '2px solid #cbd5e1', background: '#ffffff', color: '#475569', fontSize: '0.85rem', fontWeight: 800, cursor: 'pointer' }}
          ...
          <button
            onClick={() => submitPhDailyReport()}
            disabled={isSubmitting}
            style={{ flex: 1, padding: '12px', borderRadius: '14px', border: 'none', background: 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)', color: '#ffffff', fontSize: '0.85rem', fontWeight: 800, cursor: isSubmitting ? 'not-allowed' : 'pointer', boxShadow: '0 4px 12px rgba(37,99,235,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
            onMouseEnter={e => { if (!isSubmitting) { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 6px 16px rgba(37,99,235,0.35)'; } }}
            onMouseLeave={e => { if (!isSubmitting) { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(37,99,235,0.25)'; } }}
          >
            {isSubmitting ? (
              <Fragment><Loader2 size={16} className="animate-spin" /><span>กำลังส่งรายงาน...</span></Fragment>
            ) : (
