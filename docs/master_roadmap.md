...
- [X] T-310: S1 timestamp/version foundation (updatedAt/updatedBy stamp @WorkOrderContext:1325-26 + adminOpenedAt baseline @2267) · attempts:1
- [X] T-311: S2 server-side CAS guard in saveEvaluation (expectedVersion → StaleDataError export@14 · CAS@858-875 with by!==me filter) · attempts:1
- [X] T-312: S3 client-side stale guard in Evaluation (baseline ref@65/83 + banner + block approve@464 + ack@89) · attempts:1
- [X] T-313: S4 full audit trail (logService.trackChange before/after @WorkOrderContext:1344 + decision log @Evaluation:495) · attempts:1
- [X] T-314: S5 full verification (npx tsc --noEmit EXIT 0 · 0 errors · guard-wiring trace ✅) · attempts:1

- [X] T-320: Pessimistic lock-on-open — S1 remove Workstream B (CAS/banner/StaleDataError) · tool_calls:~12
- [X] T-321: S2 evalLocked flag + lifecycle (set on admin open · clear on decision leaving Evaluating)
- [X] T-322: S3 server-side lock enforcement (assertNotEvalLocked in updateTask/addTaskUpdate · daily-report submit inherits via addTaskUpdate · exempt Admin/Manager/Approver)
- [X] T-323: S4 UX lock enforcement (evalLock folded into isReadOnly · disable foreman edit + "แอดมินกำลังตรวจสอบ" banner)
- [X] T-324: S5 full verification (tsc EXIT 0 · open->lock->reject->unlock trace confirmed)

- [X] T-331: remove dead staff-collection code (3 orphan files: staffSystemMigration/migrationService/check_notifs) · git rm + index_files.json sync · tsc: 0 new errors · attempts:1 · tool_calls:9

- [X] T-332: replace MOCK_STAFF with real users in WorkOrderGroupList + delete dead code (DailyReportModal, components/SLAMonitor, mockData.ts) + index sync · tsc: 0 new errors · attempts:1 · tool_calls:11

- [X] T-335: collectionGroup delta + flat cache full port (S1-S6) · 3 new files (workOrderCacheStore/useRealtimeWorkOrders/assembleWorkOrders) + WorkOrderContext rewired + 4-listener leak fixed · kills per-WO full-tree getDocs re-reads + touch-refresh · tsc=67 · runtime /dashboard PASS · attempts:1

- [X] T-344: fix 2 dashboard SLA-card label bugs (found during T-343 in-app verify) — (1) on-time card subtitle "จาก N รายการที่เสร็จ" denominator was stats.closed (status Complete pop.) → changed to slaOnTime+slaLate (the SLA-graded pop.) so it matches the gauge/cards under the DUAL model; (2) gauge subtitle wording "มีงาน late N" → "งานค้างเกินกำหนด N" (was confusable with the finished-late "เลย SLA" card — different metric: urgentTasks critical=open-overdue vs finished-late). tsc=70 baseline. src/pages/Dashboard.tsx lines ~3014/3083. NOTE: the T-343 "late task not showing" report was a STALE BUILD (app ran pre-T-343 status===Complete gate); rebuild → correct (gauge 60%=3/5, เลย SLA=2). attempts:1 · awaiting in-app verify after rebuild.
- [X] T-336: fix 3 /dashboard bugs — unify month-window to carried-over-aware (filteredData==availableProjectsThisMonth, fixes filter interaction + WH project vanishing) + urgent-only label on "เร่งด่วน SLA" card · S1-S2 · src/pages/Dashboard.tsx · VERIFIED in-app: all 3 fixes present in code (predicates 1294-1300==1252-1254 match; urgent subtext at 2648). During verify user found 3 new symptoms: #1 per-foreman sum>total + #2 monthly sum>yearly = BY DESIGN (multi-owner + carried-over, kept) · #3 future-month leak = new bug → T-345. attempts:1
- [X] T-345: fix /dashboard future-month data leak (found during T-336 verify) — open (never-completed) WOs leak into FUTURE months because 3 "active-in-window" predicates treat open WO as active from creation onward with no upper bound at now. Fix: bound open branch with `nowMs >= startOfWindow` in getDashboardStats.carriedOverData (1449-60), filteredData (1303), availableProjectsThisMonth (1255-56). NOT healthCardProjects/projectTrend (completion-based, future empty naturally). Future months → empty; past/current unchanged. src/pages/Dashboard.tsx · S1-S3 · VERIFIED in-app: Aug empty, Jul/Jun counts unchanged, symptoms 1-2 unchanged · tsc 70 baseline held · attempts:1

[X] T-338: fix /dashboard subtask-card "ทำทั้งหมด" undercount — card left number = stats.totalTasksInScope (grand total in-scope excl Draft/Cancelled) instead of closed+open · verified in app (18→28) · attempts:1 · files: src/pages/Dashboard.tsx, src/types/dashboard.ts
[X] T-339: read-only audit scripts/audit-wo-status.mjs — root cause of wrong counts = legacy stale statuses (75% of WOA/WOP tasks: upcoming/completed/in-progress/for-checking not in current enum). Decision: NO migration, verify via new WO. attempts:1

- [X] T-340: /dashboard card scope alignment — WO gate (isWoOutOfScope) counts partial-approval Evaluating WOs; totalTasksInScope counts only APPROVED_TASK_STATUSES (excl Evaluating/Rejected). Code done, tsc +0, awaiting user in-app verify.

- [X] T-341: fix task duplication on approve/reject (Option B — immutable IDs). Root cause: global taskCounter in formatCategoriesAndTasks (WorkOrderContext.tsx:132) regenerated diverging task ids on saveEvaluation -> cleanup kept old + write added new = dup (proven via audit-id-remap.mjs). Fix: saveEvaluation:492 uses input categories (stored ids) instead of regenerating. tsc 70=baseline. Awaiting user in-app verify. attempts:1

- [X] T-342: SLA card completion gate — final = `t.status === 'Complete'` ONLY (healthCardProjects, Dashboard.tsx:1903). Gate keys on CUSTOMER ACCEPTANCE (status Complete, WorkOrderContext.tsx:1508) = process truly done, NOT foreman work-finished (dailyProgress===100). Side-effect: legacy lowercase 'completed' != 'Complete' so stale test data drops out of the card on its own. Other gates (1478/1712/1728) untouched. tsc 70=baseline. User verified in-app. NOTE: user reversed mid-task — interim state was `dailyProgress===100`, corrected to `status==='Complete'`. attempts:1
- [X] T-343: Dashboard SLA + completion model consolidation (SUPERSEDES T-342). DONE: S1 pure computeTaskSLA() helper (module scope) → S2 healthCardProjects gate=eligible + completion-month window + removed `now` bug/created-window filter → S3 getDashboardStats formula A slaScore via helper (Exec Summary == gauge) → S4 projectTrend graded by completion month → S5 count cards verified unchanged (status==='Complete'). tsc=70 baseline held every section. attempts:1 · scope: src/pages/Dashboard.tsx only. Awaiting user in-app verify. User-locked DUAL model: (1) count cards "เสร็จแล้ว" = status==='Complete' (customer accepted, unchanged); (2) SLA on-time/late cards + gauge = gate dailyProgress===100 (foreman work done — REVERTS T-342). SLA: start=startDate, end=completedAt (progress-100 date; WorkOrderContext:823/837, cleared 1582), duration=max(calendar,work), onTime=duration<=limit. Window: workload=active-in-window (carried-over incl.), SLA-performance=completion month (month completedAt). Plan: central computeTaskSLA() helper → point healthCardProjects (C, 1877-1941) + stats.slaScore (A, 1435-1490) + projectTrend (D, 1943-1996) at it; fix formula-C `now` end-fallback bug (1911); count cards verify-only (S5). Sections S1-S5. No migration, no DB, no WorkOrderContext writes. tsc 70=baseline.
