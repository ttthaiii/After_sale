...
const SLAMonitor = () => {
    ...
    const getSLARemaining = (task: any, woCreatedAt: string) => {
        // โ… 1. If task is completed (dailyProgress 100%), stop SLA
        if (task.dailyProgress === 100) {
            return { text: 'เน€เธชเธฃเนเธเธชเธดเนเธเนเธฅเนเธง', isCritical: false, isWarning: false, isDone: true, diffMs: 0 };
        }

        // โ… 2. If task is rejected, show specific status
        if (task.status === 'Rejected') {
            return { text: 'เธ–เธนเธเธเธเธดเน€เธชเธ', isCritical: false, isWarning: false, isRejected: true, diffMs: 0 };
        }
        ...
        if (diffMs < 0) {
            ...
            let text = `เน€เธเธดเธเธเธณเธซเธเธ” ${remHoursOverdue}เธเธก.`;
            if (daysOverdue > 0) text = `เน€เธเธดเธเธเธณเธซเธเธ” ${daysOverdue}เธงเธฑเธ ${remHoursOverdue}เธเธก.`;
            return { text, isCritical: true, isWarning: false, diffMs };
        }

        const hours = Math.floor(diffMs / (1000 * 60 * 60));
        let text = `เน€เธซเธฅเธทเธญ ${hours}เธเธก.`;
        if (hours >= 24) {
            const days = Math.floor(hours / 24);
            const remHours = hours % 24;
            text = `เน€เธซเธฅเธทเธญ ${days}เธงเธฑเธ ${remHours}เธเธก.`;
        }
        ...
        <div style={{ padding: '0 1rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                    ...
                    <div>
                        <h1 style={{ margin: 0, fontSize: '1.8rem', fontWeight: 900, color: '#0f172a', letterSpacing: '-0.025em' }}>เธ•เธดเธ”เธ•เธฒเธกเธชเธ–เธฒเธเธฐเธเธฒเธ (Real-time Tracking)</h1>
                        <span style={{ color: '#64748b', fontSize: '0.95rem', marginTop: '4px', display: 'block', fontWeight: 500 }}>เธกเธญเธเธดเน€เธ•เธญเธฃเนเธเธงเธฒเธกเธเธทเธเธซเธเนเธฒเนเธฅเธฐเธเธฃเธฐเน€เธกเธดเธเธเธฅเธเธฒเธเธ•เธฒเธก SLA</span>
                    </div>
                </div>
            </div>
            ...
            <div style={{ background: 'white', padding: '1.25rem', borderRadius: '24px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', marginBottom: '1.25rem', border: '1px solid #f1f5f9' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(200px, 1.2fr) 0.8fr 1fr 1fr 1fr 1fr auto', gap: '0.8rem', alignItems: 'center' }}>
                    <div style={{ position: 'relative' }}>
                        ...
                        <input type="text" placeholder="เธเนเธเธซเธฒเน€เธฅเธเธ—เธตเนเธเธฒเธ เธซเธฃเธทเธญ เธเนเธฒเธเน€เธฅเธเธ—เธตเน..." style={{ ...commonInputStyle, paddingLeft: '44px', width: '100%', boxSizing: 'border-box' }} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    </div>
                    <select style={commonInputStyle} value={selectedProjectId} onChange={e => setSelectedProjectId(e.target.value)}>
                        <option value="">เธ—เธธเธเนเธเธฃเธเธเธฒเธฃ</option>
                    ...
                    <select style={commonInputStyle} value={activeSlaFilter || ''} onChange={e => setActiveSlaFilter(e.target.value as any || null)}>
                        <option value="">เธชเธ–เธฒเธเธฐ SLA เธ—เธฑเนเธเธซเธกเธ”</option>
                        <option value="overdue">๐”ด เน€เธเธดเธเธเธณเธซเธเธ”</option>
                        <option value="warning">๐ก เธงเธดเธเธคเธ• (&lt;24เธเธก.)</option>
                        <option value="normal">๐”ต เธเธเธ•เธด (&gt;24เธเธก.)</option>
                        <option value="completed">๐ข เน€เธชเธฃเนเธเธชเธดเนเธ</option>
                    </select>

                    {currentRole !== 'Foreman' ? (
                        <select style={commonInputStyle} value={selectedStaffId} onChange={e => setSelectedStaffId(e.target.value)}>
                            <option value="">เน€เธเนเธฒเธซเธเนเธฒเธ—เธตเนเธ—เธฑเนเธเธซเธกเธ”</option>
                    ...
                    <div style={{ display: 'flex', background: '#f8fafc', padding: '4px', borderRadius: '14px', border: '1px solid #e2e8f0' }}>
                        ...
                        >
                            เน€เธงเธฅเธฒเน€เธเธดเธ”เนเธเธเธฒเธ
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', minHeight: 200 }}>
                                {columnTasks.length === 0 ? (
                                    ...
                                    columnTasks.map(task => {
                                        ...
                                        const assignedName = assignedStaff?.name || assignedContractor?.name || 'เธขเธฑเธเนเธกเนเธกเธญเธเธซเธกเธฒเธข';
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {isExpanded && (
                                                    ...
                                                    >
                                                        <div>
                                                            <span style={{ fontWeight: 800, color: '#94a3b8' }}>เธเธนเนเธฃเธฑเธเธเธดเธ”เธเธญเธ:</span> <span style={{ fontWeight: 900, color: '#1e293b' }}>{assignedName} ({assignedRole})</span>
                                                        </div>
                                                        <div>
                                                            <span style={{ fontWeight: 800, color: '#94a3b8' }}>เน€เธเธญเธฃเนเธ•เธดเธ”เธ•เนเธญ:</span> <span style={{ fontWeight: 900, color: '#1e293b' }}>{assignedPhone}</span>
                                                        </div>
                                                        <div>
                                                            <span style={{ fontWeight: 800, color: '#94a3b8' }}>เธงเธฑเธเน€เธฃเธดเนเธกเธเธฒเธ:</span> <span style={{ fontWeight: 900, color: '#1e293b' }}>{task.startDate ? new Date(task.startDate).toLocaleDateString('th-TH') : '-'}</span>
                                                        </div>
                                                        {task.history && task.history.length > 0 && (
                                                            <div style={{ background: '#f8fafc', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', marginTop: '4px' }}>
                                                                <div style={{ fontWeight: 900, color: '#475569', marginBottom: '4px', fontSize: '0.75rem' }}>เธซเธกเธฒเธขเน€เธซเธ•เธธเธฅเนเธฒเธชเธธเธ”:</div>
                                                                <div style={{ fontStyle: 'italic', color: '#64748b' }}>"{task.history[0].note || 'เนเธกเนเธกเธตเธเนเธญเธเธงเธฒเธกเธซเธกเธฒเธขเน€เธซเธ•เธธ'}"</div>
                                                            </div>
                                                        )}
                                                        ...
                                                        <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                                                            ...
                                                            {(currentRole === 'Admin' || currentRole === 'Manager' || currentRole === 'Approver') && (
                                                                ...
                                                                >
                                                                    <RotateCw size={12} />
                                                                    {task.responsibleStaffIds && task.responsibleStaffIds.length > 0 ? 'เน€เธเธฅเธตเนเธขเธเธเธนเนเธฃเธฑเธเธเธดเธ”เธเธญเธ' : 'เธกเธญเธเธซเธกเธฒเธข'}
                                                                </button>
                                                            )}
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f1f5f9', paddingTop: '12px', marginTop: '12px' }}>
                                                    ...
                                                    <div style={{ fontSize: '0.7rem', fontWeight: 800, color: '#94a3b8' }}>
                                                        {isExpanded ? 'เธเธฅเธดเธเน€เธเธทเนเธญเธขเธธเธ' : 'เธเธฅเธดเธเน€เธเธทเนเธญเธเธขเธฒเธข'}
                                                    </div>
                                                </div>
                                            </div>
                                        );
